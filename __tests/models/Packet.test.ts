import mockingoose from "mockingoose";
import PacketsModel from "../../src/models/Packets";

describe("test mongoose User model", () => {
  test("should return the doc with findById", () => {
    const _doc = { likes: 0, views: 0, status: 1, comments: [] as any[] };

    mockingoose(PacketsModel).toReturn(_doc, "findOne");

    return PacketsModel.findById({ _id: "507f191e810c19729de860ea" }).then(
      (doc) => {
        expect(JSON.parse(JSON.stringify(doc))).toMatchObject(_doc);
      }
    );
  });

  // test('should return the doc with update', () => {
  //   const _doc = {
  //     _id: '507f191e810c19729de860ea',
  //     name: 'name',
  //     email: 'name@email.com',
  //   };

  //   mockingoose(model).toReturn(doc, 'update');

  //   return model
  //     .update({ name: 'changed' }) // this won't really change anything
  //     .where({ _id: '507f191e810c19729de860ea' })
  //     .then(doc => {
  //       expect(JSON.parse(JSON.stringify(doc))).toMatchObject(_doc);
  //     });
  // });
});
